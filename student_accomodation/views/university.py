from student_accomodation.serializers.universityserializers import UniversitySerializer,UniversityCitySerializer
...
class universitycity(generics.ListCreateAPIView):
    queryset=University.objects.select_related('added_by','country','city').annotate(added_by_name=F('added_by__first_name'),country_name=F('country__name'),city_name=F('city__name')).filter(is_enabled=1).all()
    serializer_class=UniversityCitySerializer